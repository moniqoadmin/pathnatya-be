...
export class AccountLogsController {
  ...
  @ApiOperation({
    summary:
      'List log events for an account (paginated, newest first). Optional admin query flag. Users may view their own logs. Admins may view User accounts in their sanghat. SuperAdmin and Developer may view any account.',
  })
  findAllForAccount(
    @Req() req: Request,
    ...
    return this.logsService.findAllForAccountId(
      req.user!.sub,
      accountId,
      query,
    );
